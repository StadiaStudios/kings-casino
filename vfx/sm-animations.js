(function (global) {
    ...
    function ensureChipFallAndJackpotStyles() {
        if (!document.getElementById('chip-fall-dark-style')) {
            const style = document.createElement('style');
            style.id = 'chip-fall-dark-style';
            style.textContent = `
                body.sm-chip-fall-dark,
                html.sm-chip-fall-dark {
                    transition: background 0.36s cubic-bezier(.42,0,1,1), background-color 0.36s cubic-bezier(.42,0,1,1);
                    background:rgb(0, 0, 0) !important;
                    background-color:rgb(0, 0, 0) !important;
                }
                body.sm-chip-fall-silver,
                html.sm-chip-fall-silver {
                    transition: background 0.36s cubic-bezier(.42,0,1,1), background-color 0.36s cubic-bezier(.42,0,1,1);
        ...
        const observer = new MutationObserver(function(mutations) {
            ...
            if (chipsAdded) {
                ...
                if (jackpotFallActive) {
                    document.body.classList.remove('sm-chip-fall-dark');
                    document.documentElement.classList.remove('sm-chip-fall-dark');
                    document.body.classList.add('sm-chip-fall-silver');
                    document.documentElement.classList.add('sm-chip-fall-silver');
                } else {
                    document.body.classList.remove('sm-chip-fall-silver');
                    document.documentElement.classList.remove('sm-chip-fall-silver');
                    document.body.classList.add('sm-chip-fall-dark');
                    document.documentElement.classList.add('sm-chip-fall-dark');
                }
            }

            if (chipShower.querySelectorAll('.falling-chip').length === 0 && chipsRemoved) {
                setTimeout(() => {
                    document.body.classList.remove('sm-chip-fall-dark', 'sm-chip-fall-silver');
                    document.documentElement.classList.remove('sm-chip-fall-dark', 'sm-chip-fall-silver');
    ...
    function triggerJackpotShake(duration = 800) {
        ...
        if (!document.getElementById('sm-jackpot-shake-style')) {
            const style = document.createElement('style');
            style.id = 'sm-jackpot-shake-style';
            style.textContent = `
                @keyframes sm-jackpot-circle-shake {
                ...
                .sm-jackpot-shake {
                    animation: sm-jackpot-circle-shake 0.7s cubic-bezier(.35,1.3,.62,1.02) both;
        ...
        target.classList.remove('sm-jackpot-shake');
        void target.offsetWidth;
        target.classList.add('sm-jackpot-shake');
        setTimeout(() => {
            target.classList.remove('sm-jackpot-shake');
            document.body.classList.remove('sm-chip-fall-silver');
            document.documentElement.classList.remove('sm-chip-fall-silver');
